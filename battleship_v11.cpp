    ...
    prntBd(true, board);
     ...
     atkAc = accCal(static_cast<float>(totHit),totShot);
        ...
        if(hit){
            ...
            if(remShip==4)cout<<"You hit your first ship! Way to go!"<<endl<<endl;
            if(remShip==3)cout<<"Three opponent ships left!"<<endl<<endl;
            else cout<<"Great job!"<<endl<<endl;
            cout<<(remShip==1?"One ship left!":"Keep firing!")<<endl<<endl;
        }else{
            cout<<"You did not hit any of your opponent's ships."<<endl<<endl;
            cout<<"To launch another attack, repeat the same process."<<endl<<endl;
        }
    }  
